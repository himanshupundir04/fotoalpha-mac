...
function CreateEvents() {
  ...
  const [verificationDialog, setVerificationDialog] = useState({ open: false, status: "", title: "", text: "" });
  const [upgradeDialog, setUpgradeDialog] = useState({ open: false });

  // Fetch verification status and event count on mount
  // useEffect(() => {
  //   // Fetch event count first, then check verification status
  //   fetchEventCount().then((count) => {
  //     checkVerificationStatus(count);
  //   });
  // }, []);
  ...
  const checkVerificationStatus = async (currentEventCount) => {
    ...
    try {
      ...
      const status = response?.data?.latestSubmission?.status || response?.data?.status || "idle";
      setVerificationStatus(status);
      setIsCheckingVerification(false);

      // Only show verification popup if user has already created at least 1 event AND verification is not approved
      if (status !== "approved" && currentEventCount >= 1) {
        showVerificationPopup(status);
      }
    } catch (error) {
      console.error("Error fetching verification status:", error);
      setVerificationStatus("idle");
      setIsCheckingVerification(false);
      // Show popup on error only if user has already created at least 1 event
      if (currentEventCount >= 1) {
        showVerificationPopup("idle");
      }
    }
  };
    ...
    return (
      ...
      >
        <Stack alignItems="center" spacing={2}>
          ...
          <Typography color="text.secondary">Checking verification status...</Typography>
        </Stack>
      </Box>
    );
  }
                ...
                <Stack spacing={0.5}>
                  ...
                  {showStep2Errors && formik.errors.hostMobile && (
                    <Typography
                      variant="caption"
                      color="#e43838"
                      sx={{ fontSize: "0.74rem", paddingLeft: "12px" }}
                    >
      ...
      >
        <DialogTitle sx={{ 
          ...
          <Box sx={{ 
            ...
            height: 44,
            borderRadius: '50%',
            backgroundColor: verificationDialog.status === 'rejected' ? '#FEE2E2' : verificationDialog.status === 'pending' ? '#FEF3C7' : '#DBEAFE',
          }}>
            {verificationDialog.status === 'rejected' ? (
              <ErrorOutlineIcon sx={{ color: '#EF4444', fontSize: 24 }} />
            ) : (
              <WarningAmberIcon sx={{ color: verificationDialog.status === 'pending' ? '#F59E0B' : '#3B82F6', fontSize: 24 }} />
            )}
          </Box>
        ...
        <DialogContent sx={{ px: 3, pb: 2 }}>
          <DialogContentText sx={{ color: '#6B7280', fontSize: '0.9rem', lineHeight: 1.6 }}>
            {verificationDialog.text}
          </DialogContentText>
        </DialogContent>
        <DialogActions sx={{ px: 3, pb: 2.5, gap: 1 }}>
          <Button
            onClick={() => handleVerificationDialogClose(false)}
            variant="outlined"
            sx={{
              borderColor: '#E5E7EB',
              color: '#374151',
          ...
          <Button
            onClick={() => handleVerificationDialogClose(true)}
            variant="contained"
            sx={{
              backgroundColor: '#3B82F6',
              textTransform: 'none',
      ...
      >
        <DialogTitle sx={{ 
          ...
          <Box sx={{ 
            ...
            <WarningAmberIcon sx={{ color: '#3B82F6', fontSize: 24 }} />
          </Box>
          <Typography variant="h6" fontWeight={600} color="#111827">
            Upgrade Your Plan
          </Typography>
        ...
        <DialogContent sx={{ px: 3, pb: 2 }}>
          <DialogContentText sx={{ color: '#6B7280', fontSize: '0.9rem', lineHeight: 1.6 }}>
            You need to upgrade your plan to perform this action. Upgrade now to unlock more features and create unlimited events.
          </DialogContentText>
        </DialogContent>
        <DialogActions sx={{ px: 3, pb: 2.5, gap: 1 }}>
          <Button
          ...
          <Button
            onClick={() => {
              ...
              navigate("/photographer/upgrade_plan");
            }}
            variant="contained"
            sx={{
              backgroundColor: '#3B82F6',
