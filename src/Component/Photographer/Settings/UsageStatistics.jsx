...
const UsageStatistics = () => {
  ...
  if (
    ...
    return (
      <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-xl p-5 text-red-800 dark:text-red-300 mt-5">
  ...
  if (!hasUsageData) {
    return (
      <div className="relative overflow-hidden rounded-2xl border border-slate-200 dark:border-slate-700 bg-gradient-to-br from-slate-50 via-white to-blue-50 dark:from-slate-900 dark:via-slate-800 dark:to-slate-900 p-8 mt-5">
  ...
  return (
    <div className="space-y-4 text-start">
      {/* Subscription Plan */}
      <div className="bg-gradient-to-r from-blue-50 to-purple-50 dark:from-slate-800/50 dark:to-slate-700/50 rounded-lg p-4 border border-blue-200 dark:border-slate-700/50">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-2.5">
        {/* Events Card */}
        <div className="bg-white dark:bg-slate-800/50 rounded-lg p-2.5 border border-blue-200 dark:border-slate-700/50 shadow-md">
          ...
          <div className="space-y-1.5">
            <div>
              <p className="text-[10px] text-slate-600 dark:text-slate-400">Used</p>
            ...
            <div className="bg-slate-100 dark:bg-slate-700 rounded-full h-1.5 overflow-hidden">
              <div
                ...
                style={{ width: `${events?.available > 0 ? (events?.used / events?.available) * 100 : 0}%` }}
              />
            </div>
        ...
        {/* Photo Uploads Card */}
        <div className="bg-white dark:bg-slate-800/50 rounded-lg p-2.5 border border-yellow-200 dark:border-slate-700/50 shadow-md">
          <div className="flex items-center justify-between mb-2">
            <h3 className="font-semibold text-slate-900 dark:text-white text-xs">
              Photo Uploads
            </h3>
          ...
          <div className="space-y-1.5">
            ...
            <div className="bg-slate-100 dark:bg-slate-700 rounded-full h-1.5 overflow-hidden">
              <div
                ...
                style={{ width: `${photo_uploads?.available > 0 ? (photo_uploads?.used / photo_uploads?.available) * 100 : 0}%` }}
              />
            </div>
            <p className="text-[10px] text-slate-600 dark:text-slate-400">
        ...
        {/* Storage Card */}
        <div className="bg-white dark:bg-slate-800/50 rounded-lg p-2.5 border border-green-200 dark:border-slate-700/50 shadow-md">
          <div className="flex items-center justify-between mb-2">
            <h3 className="font-semibold text-slate-900 dark:text-white text-xs">
              Storage
            </h3>
          ...
          <div className="space-y-1.5">
            ...
            <div className="bg-slate-100 dark:bg-slate-700 rounded-full h-1.5 overflow-hidden">
              <div
                ...
                style={{ width: `${storage?.available > 0 ? (storage?.used / storage?.available) * 100 : 0}%` }}
              />
            </div>
            <p className="text-[10px] text-slate-600 dark:text-slate-400">
        ...
        {/* Team Members Card */}
        <div className="bg-white dark:bg-slate-800/50 rounded-lg p-2.5 border border-purple-200 dark:border-slate-700/50 shadow-md">
          <div className="flex items-center justify-between mb-2">
            <h3 className="font-semibold text-slate-900 dark:text-white text-xs">
              Team Members
            </h3>
          ...
          <div className="space-y-1.5">
            <div>
              <p className="text-[10px] text-slate-600 dark:text-slate-400">Added</p>
            ...
            <div className="bg-slate-100 dark:bg-slate-700 rounded-full h-1.5 overflow-hidden">
              <div
                ...
                style={{ width: `${team_members?.available > 0 ? (team_members?.used / team_members?.available) * 100 : 0}%` }}
              />
            </div>
            <p className="text-[10px] text-slate-600 dark:text-slate-400">
      ...
      {addons && addons.total > 0 && (
        <div className="bg-white dark:bg-slate-800/50 rounded-lg p-4 border border-indigo-200 dark:border-slate-700/50 shadow-md">
          <div className="flex items-center justify-between mb-2.5">
            <h3 className="text-sm font-semibold text-slate-900 dark:text-white flex items-center gap-1.5">
              <span className="text-base">🎁</span> Add-ons ({addons.active} active)
            </h3>
          </div>
            ...
            {addons.data.map((addon, index) => (
              ...
              >
                <div className="flex-1">
                  ...
                  <div className="flex items-center gap-2.5 mt-1 text-[10px] text-slate-600 dark:text-slate-400">
                    <span>💰 ₹{addon.amountRupees}</span>
                    <span>⏰ {new Date(addon.createdAt).toLocaleDateString()}</span>
                    {addon.expiresAt && (
                      <span className={addon.isExpired ? "text-red-600 dark:text-red-400" : ""}>
                        Expires: {new Date(addon.expiresAt).toLocaleDateString()}
                      </span>
                    )}
                  </div>
                  {addon.type !== "subscription_discount" && addon.remaining >= 0 && (
                    <div className="mt-1.5">
                      <div className="flex justify-between items-center mb-0.5">
                        <span className="text-[10px] text-slate-600 dark:text-slate-400">Remaining</span>
                      ...
                      <div className="w-full bg-slate-200 dark:bg-slate-700 rounded-full h-1">
                        <div
                          ...
                          style={{ width: `${addon.quantity > 0 ? (addon.remaining / addon.quantity) * 100 : 0}%` }}
                        />
                      </div>
                    </div>
                  )}
                ...
                <div className="ml-2.5 flex-shrink-0">
                  {addon.status === "confirmed" && !addon.isExpired ? (
                    ...
                    <span className="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-medium bg-gray-100 dark:bg-gray-800 text-gray-800 dark:text-gray-300">
                      Inactive
                    </span>
                  )}
                </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        {/* Bar Chart */}
        <div className="bg-white dark:bg-slate-800/50 rounded-lg p-4 border border-slate-200 dark:border-slate-700/50 shadow-md">
        ...
        <div className="bg-white dark:bg-slate-800/50 rounded-lg p-4 border border-slate-200 dark:border-slate-700/50 shadow-md">
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between pb-2 border-b border-slate-200 dark:border-slate-700">
              <span className="text-xs text-slate-600 dark:text-slate-400">Events Used</span>
              <span className="text-xs font-semibold text-slate-900 dark:text-white">
                {events?.used || 0} / {events?.available || 0}
              </span>
            </div>
            <div className="flex items-center justify-between pb-2 border-b border-slate-200 dark:border-slate-700">
              <span className="text-xs text-slate-600 dark:text-slate-400">Photos Uploaded</span>
              <span className="text-xs font-semibold text-slate-900 dark:text-white">
                {photo_uploads?.used || 0} / {photo_uploads?.available || 0}
              </span>
            </div>
            <div className="flex items-center justify-between pb-2 border-b border-slate-200 dark:border-slate-700">
              <span className="text-xs text-slate-600 dark:text-slate-400">Storage Used</span>
              <span className="text-xs font-semibold text-slate-900 dark:text-white">
                {(storage?.used || 0).toFixed(2)} / {(storage?.available || 0).toFixed(2)} GB
              </span>
            </div>
