...
function CreateEvents() {
  ...
  const checkVerificationStatus = async () => {
    try {
      ...
      const status =
        response?.data?.latestSubmission?.status ||
        response?.data?.status ||
        "idle";
      setVerificationStatus(status);
      setIsCheckingVerification(false);

      // If not approved, show popup
      if (status !== "approved") {
  ...
  const fetchGuar = async () => {
    ...
    try {
      const response = await axios.get(
        `${baseURL}/v1/subscription/guard/event`,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`,
          },
  ...
  const showVerificationPopup = (status) => {
    ...
    switch (status) {
      case "pending":
        ...
        text =
          "Your ID verification is currently under review. You cannot create events until your verification is approved.";
        break;
      case "rejected":
        title = "Verification Rejected";
        text =
          "Your ID verification was rejected. Please submit valid documents to create events.";
        break;
      default:
        title = "Verification Required";
        text =
          "You need to verify your ID before creating events. Please complete your verification first.";
    }
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      try {
        ...
        setEventname("");
        setTimeout(() => {
          navigate("/photographer/events_list");
        }, 1000);
      } catch (error) {
        setLoading(false);
        // console.error(error);
        ...
        >
          ...
          >
            {!canCreateEvent ? (
              ...
              >
                <WarningAmberIcon
                  sx={{ fontSize: 48, color: "#EF4444", mb: 2 }}
                />
                <Typography
                  variant="h6"
                  color="#DC2626"
                ...
                <Typography variant="body2" color="#7F1D1D" mb={2}>
                  You have reached the maximum number of events allowed on your
                  current plan. Please upgrade to create more events.
                </Typography>
                <Button
                  variant="contained"
                  onClick={() => navigate("/photographer/upgrade_plan")}
                    ...
                    <Stack spacing={1.5}>
                      ...
                      <Stack spacing={0.5}>
                        ...
                        <TextField
                          ...
                          error={
                            showStep2Errors && Boolean(formik.errors.hostName)
                          }
                          helperText={
                            showStep2Errors ? formik.errors.hostName : ""
                          }
                          size="small"
                          sx={{
                            "& .MuiFilledInput-root": {
      ...
      >
        ...
        <DialogContent sx={{ px: 3, pb: 2 }}>
          <DialogContentText
            sx={{ color: "#6B7280", fontSize: "0.9rem", lineHeight: 1.6 }}
          >
            You need to upgrade your plan to perform this action. Upgrade now to
            unlock more features and create unlimited events.
          </DialogContentText>
        </DialogContent>
        <DialogActions sx={{ px: 3, pb: 2.5, gap: 1 }}>
          <Button
