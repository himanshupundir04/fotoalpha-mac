...
function Images({ folders, fetchFolder }) {
  ...
  const [sliderHoverIndex, setSliderHoverIndex] = useState(null);
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios
          ...
          .then(() => {
            toast.success("Folder deleted successfully", { autoClose: 1000 });
            setOpenfolder(false);
            fetchFolder();
            setInitialLoaded(false);
            pageRef.current = 1;
  ...
  return (
    <>
      ...
      <div className="bg-white p-4 rounded dark:bg-slate-800 mt-4 text-start">
              ...
              <div className="grid grid-cols-3 sm:grid-cols-3 md:grid-cols-5 gap-5">
                ...
                ${isSelected ? "scale-105" : "hover:scale-105"}`}
                      ...
                      >
                        ...
                        <div className="absolute top-2 right-2 z-50 hidden group-hover:block">
                          <DeleteIcon
                            className="text-red-600 cursor-pointer hover:text-red-800 bg-white rounded-full p-1"
                            style={{ fontSize: "22px" }}
                            onClick={(e) => {
                              e.stopPropagation();
                    ...
                    >
                      <input
                        ...
                        className={`absolute z-20 top-2 left-2 ${
                          selectDelete.includes(photo?._id)
                            ? "block"
                      ...
                      />
                      <span
                        className="absolute top-0 p-2 left-0 rounded font-normal text-xl w-full h-full hover:bg-black/30 shadow ease-in-out duration-300"
                        onClick={() => {
                      ...
                      ></span>
                      <div className="absolute top-1 right-1 z-20 bg-slate-50/30 rounded-full">
                        <div>
                          <IconButton
                            className="p-0"
                            aria-label="more"
                          ...
                          >
                            ...
                            >
                              <DeleteIcon className="me-2 text-red-600" />{" "}
              ...
              {photosFolder.map((photo, index) => (
                <>
                  ...
                  <div 
                    ...
                    onMouseEnter={() => setSliderHoverIndex(index)}
                    onMouseLeave={() => setSliderHoverIndex(null)}
                  >
                    <img
                      src={photo.signedUrl}
                      alt={photo.caption || "Photo"}
                      className="w-max max-w-[80vw] h-[80vh] md:max-h-[80vh] md:max-w-[80vw] object-contain m-auto "
                      loading="lazy"
                      onLoad={() => handleImageLoad(index)}
                      key={index}
                    />
                    {/* Delete icon overlay - always rendered but visible on hover */}
                    <div 
                      className={`absolute top-2 right-2 z-50 transition-opacity duration-200 ${sliderHoverIndex === index ? 'opacity-100' : 'opacity-0'}`}
                    >
                      <DeleteIcon
                        className="text-red-600 cursor-pointer hover:text-red-800 bg-white rounded-full p-1"
                        style={{ fontSize: '32px' }}
