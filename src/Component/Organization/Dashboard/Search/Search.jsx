...
function Search() {
  ...
  const fecthSearch = () => {
    setloading(true);
    axios
      .get(`${baseURL}/photographer/search/${name}`, {
        headers: {
  ...
  const columns = [
    {
      ...
      options: {
        ...
        customBodyRender: (value) => {
          return (
            <span className="dark:text-slate-400 capitalize">{value}</span>
          );
        },
      },
    },
    {
      ...
      options: {
        filter: true,
        sort: true,
        customBodyRender: (slots) => {
          if (!Array.isArray(slots) || slots.length === 0 || !slots[0]?.date) {
            return <span className="dark:text-slate-400">—</span>;
          }
          ...
          return (
            <span className="dark:text-slate-400">
              {format(new Date(firstDate), "MMM dd, yyyy")}
            </span>
          );
        },
      },
    },
    {
      ...
      options: {
        ...
        customBodyRender: (value) => {
          ...
          return (
            <span
              className={`${bgColor} ${textColor} border ${border} px-2 py-1 rounded-full font-medium`}
            >
              {value}
            </span>
          );
    ...
    },
    {
      ...
      options: {
        ...
        viewColumns: false,
        customBodyRender: (value) => {
          return <span className="dark:text-slate-400">{value}</span>;
        },
      },
    },
    {
      ...
      options: {
        ...
        customBodyRender: (value) => {
          ...
          return (
            <>
              <span
                className="dark:text-slate-400 text-white px-3 py-1 rounded-full font-semibold bg-green-600 cursor-pointer"
                onClick={() => navigate(`/organization/event/${value}`)}
              >
                View
              </span>
            </>
                   ...
                   <div
                      ...
                      >
                        ...
                        <div className="flex justify-between items-center mt-2">
                          <h2 className="text-slate-700 font-normal text-center capitalize w-full dark:text-white line-clamp-2">
                            {data?.name}
                          </h2>
                          <p
