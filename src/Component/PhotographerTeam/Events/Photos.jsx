...
function Photos() {
  ...
  const fetchPhoto = async () => {
    axios
      .get(`${baseURL}/photos/get/${id}/${subid}`, {
        headers: {
                ...
                <div className="flex flex-col" key={index}>
                  <div className="max-w-sm bg-white rounded-md shadow-sm border-1 border-slate-200 dark:border-slate-700 dark:bg-slate-700 relative w-56 h-52">
                    <img
                      src={
                        selected
                          ? event?.watermarked_path
                          : event?.urlImageSignedUrl
                      }
                      alt="Event"
                      className="w-56 h-52 object-contain absolute top-0
                    left-0 rounded-md cursor-pointer"
                      loading="lazy"
                      onClick={() =>
                        setZoomImg(
                          selected
